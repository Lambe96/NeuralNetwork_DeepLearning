...
from Hype_Select_Convaut import Hype_Select_Convaut
from Conv_autoenc import Encoder, Decoder, Classifier, train_epoch, test_epoch
...
# DEFINE THE OBJECT OF THE CLASS Hype_Select_Convaut THAT PERFORM THE OPTIMIZATION
model = Hype_Select_Convaut(train_data = data_to_study_train, test_data = data_to_study_test)
...
model.Hype_sel('study_Convaut',150,43000)
...
for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(len(train_dataset)))):
    ...
    test_loader = DataLoader(train_dataset, batch_size=batch_size, sampler=test_sampler,num_workers=0)
...
if model.encoded_space_dim > 2:
    ...
    px.scatter(encoded_samples, x='Encoded Var. 0', y='Encoded Var. 1', color=[label_names[l] for l in encoded_samples['label']], opacity=0.7)
